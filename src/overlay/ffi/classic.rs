...
pub(in crate::overlay) struct NotchPanelState {
    panel: *mut c_void,
    ...
    width: f64,
    height: f64,
    pub(in crate::overlay) eq_bars: Vec<f32>,
    pub(in crate::overlay) loading_tick: usize,
}

unsafe impl Send for NotchPanelState {}
unsafe impl Sync for NotchPanelState {}

pub(in crate::overlay) fn create_notch_panel(
...
pub(in crate::overlay) fn update_notch_bars(state: &mut NotchPanelState, new_bars: &[f32]) {
    let attack = 0.6f32;
    let decay = 0.12f32;
...
pub(in crate::overlay) fn update_notch_loading(state: &mut NotchPanelState) {
    unsafe {
        let ca_transaction = objc_getClass(c"CATransaction".as_ptr());
        objc_msgSend(ca_transaction, sel_registerName(c"begin".as_ptr()));
        let msg_bool: MsgSendBool = std::mem::transmute(objc_msgSend as *const ());
...
pub(in crate::overlay) fn close_notch_panel(state: &NotchPanelState) {
    unsafe {
