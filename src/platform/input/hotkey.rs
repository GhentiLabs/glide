...
fn handle_press(ctx: &mut TapContext) {
    ...
    if microphone_status
        == crate::platform::permissions::MicrophoneAuthorizationStatus::NotDetermined
    {
        let requested_status = crate::platform::permissions::request_microphone_access();
        ctx.pressed = false;
        ctx.shared
            .set_overlay_phase(crate::app::state::OverlayPhase::Dismissed);
        if requested_status.can_capture() {
            ...
            crate::platform::permissions::microphone_access_error(requested_status)
        {
            eprintln!("[glide] {message}");
            ctx.shared.set_error();
            if requested_status.is_denied_or_restricted() {
                crate::platform::permissions::open_microphone_settings();
            }
        }
        return;
    }
    ...
    if !microphone_status.can_capture() {
        ctx.pressed = false;
        if let Some(message) =
            crate::platform::permissions::microphone_access_error(microphone_status)
        {
            eprintln!("[glide] {message}");
        } else {
    ...
    crate::engines::local_models::prewarm::start_recording_prewarm(
        ctx.shared.clone(),
        ctx.runtime.clone(),
        frontmost.clone(),
    );
    ...
    match ctx.recorder.start(&config.audio) {
        Ok(live_audio) => {
            ...
            ctx.shared
                .set_overlay_phase(crate::app::state::OverlayPhase::Recording);
        }
        Err(error) => {
            ctx.pressed = false;
...
fn handle_release(ctx: &mut TapContext) {
    ...
    trace.measure("hotkey_release_overlay_processing", || {
        ctx.shared
            .set_overlay_phase(crate::app::state::OverlayPhase::Processing)
    });
